...
def build_files():
    ...
    server_files = [
        "ADDON_NAME/server/sv_ADDON_NAME_database.lua",
        "ADDON_NAME/server/sv_ADDON_NAME_network.lua",
        "ADDON_NAME/server/sv_ADDON_NAME_functions.lua",
        "ADDON_NAME/server/sv_ADDON_NAME_hooks.lua",
        "ADDON_NAME/server/sv_ADDON_NAME_meta.lua"
    ]
    client_files = [
        "ADDON_NAME/client/cl_ADDON_NAME_network.lua",
        "ADDON_NAME/client/cl_ADDON_NAME_functions.lua",
        "ADDON_NAME/client/cl_ADDON_NAME_vgui.lua",
        "ADDON_NAME/client/cl_ADDON_NAME_hooks.lua"
    ]
    shared_files = "ADDON_NAME/config/sh_ADDON_NAME_config.lua"

    with open(resource_path("dict/start_file"), "r") as file:
        data += file.read()
    if server.get():
        with open(resource_path("dict/server_part_sv"), "r") as file:
            data += file.read()
        for files in server_files:
            open(addon_path + addon_name + "/lua/" + files.replace(search_text, addon_name), 'w+')
    if client.get():
        with open(resource_path("dict/server_part_cl"), "r") as file:
            data += file.read()
        for files in client_files:
            open(addon_path + addon_name + "/lua/" + files.replace(search_text, addon_name), 'w+')
    if shared.get():
        with open(resource_path("dict/shared_server_part"), "r") as file:
            data += file.read()
        open(addon_path + addon_name + "/lua/" + shared_files.replace(search_text, addon_name), 'w+')
    if client.get():
        with open(resource_path("dict/mid_part"), "r") as file:
            data += file.read()
        with open(resource_path("dict/client_part_cl"), "r") as file:
            data += file.read()
        if shared.get():
            with open(resource_path("dict/client_part_sh"), "r") as file:
                data += file.read()
    elif shared.get():
        with open(resource_path("dict/mid_part"), "r") as file:
            data += file.read()
    ...
    with open(resource_path("dict/end_part"), "r") as file:
        data += file.read()
...
def build_folder():
    ...
    try:
        ...
        if server.get():
            Path(addon_path + addon_name + "/lua/" + addon_name + "/server").mkdir(parents=True, exist_ok=True)
        if client.get():
            Path(addon_path + addon_name + "/lua/" + addon_name + "/client").mkdir(parents=True, exist_ok=True)
        if shared.get():
            Path(addon_path + addon_name + "/lua/" + addon_name + "/config").mkdir(parents=True, exist_ok=True)
...
def draw_window():
    ...
    Entry(top, textvariable=addon_name_var, font=('calibre', 10, 'normal')).place(x=250, y=200)
